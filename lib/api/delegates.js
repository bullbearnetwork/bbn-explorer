...
module.exports = function (app) {
	...
	function Active() {
		this.getActive = function (cb) {
			request.get({
				url: `${app.get('lisk address')}/api/delegates/?orderBy=rank:asc&limit=41`,
				json: true,
			}, (err, response, body) => {
	...
	function Standby(n) {
		...
		this.actualOffset = (isNaN(this.offset)) ? 41 : this.offset + 41;
	...
	this.getNextForgers = function (query, error, success) {
		request.get({
			url: `${app.get('lisk address')}/api/delegates/getNextForgers?limit=41`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
				return error({ success: false, error: (err || 'Response was unsuccessful') });
